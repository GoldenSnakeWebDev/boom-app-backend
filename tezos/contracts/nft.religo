...
let transfer = ((params, storage): (transfer_params_type, storage_type)): return_type => {
  let transaction_sender = Tezos.get_sender ();
  ...
  let new_ledger = List.fold(((ledger, transfer): (ledger_type, transfer_type)): ledger_type => {
    ...
    List.fold(((ledger, transfer_destination): (ledger_type, transfer_destination_type)): ledger_type => {
      ...
      let _assert = assert_with_error(is_token_supported, fa2_token_undefined_error);

      /* you can't transfer more than 1 NFT */
      let _assert = assert_with_error(is_transfering_correct_amount, fa2_tx_denied_error);
      ...
      if (transfer_amount == 0n) {
        ...
        let _assert = assert_with_error(previous_owner_account_balance == 1n, fa2_insufficient_balance_error);

        /* owner can't make transfer back to the same account */
        let _assert = assert_with_error(transfer_from != transfer_to, fa2_insufficient_balance_error);

        /* new owner should not have a token */
        let _assert = assert_with_error(new_owner_account_balance == 0n, fa2_insufficient_balance_error);
...
let update_operators = ((params, storage): (update_operators_params_type, storage_type)): return_type => {
  ...
  let new_operators: operators_type = List.fold(
    ((operators, update_operator_action): (operators_type, update_operator_action_type)): operators_type => {
      ...
      let _assert = assert_with_error(transaction_sender == update_operator_action_params.owner, fa2_not_owner_error);
...
let mint = ((params, storage): (mint_params_type, storage_type)): return_type => {
  ...
  /* Only admin can mint */
  let _assert = assert_with_error(transaction_sender == storage.admin, not_admin_error);
  ...
  let new_storage = List.fold(
    ((storage, mint_instruction): (storage_type, mint_instruction_type)): storage_type => {
      ...
      let _assert = assert_with_error(is_minting_correct_amount, fa2_insufficient_balance_error);
      ...
      if (mint_amount == 0n) {
        ...
        /* new owner should not have a token */
        let _assert = assert_with_error(account_balance == 0n, fa2_insufficient_balance_error);
        ...
        let total_supply_for_token: nr_of_tokens_type = switch (Big_map.find_opt(mint_token_id, storage.total_supply)) {
          ...
          | None () => (failwith(fa2_token_undefined_error): nr_of_tokens_type)
        };
        ...
        let next_token_id_after_minting = mint_token_id + 1n;
        ...
        /* return updated storage */
        {
          ...
          total_supply: total_supply_after_minting,
            next_token_id: next_token_id_after_minting
        };
      }
    },
    mint_instructions,
...
[@view] let token_metadata = ((params, storage): (token_id_type, storage_type)): token_metadata_value_type => {
  switch (Big_map.find_opt(params, storage.token_metadata)) {
    ...
    | None () => (failwith(fa2_token_undefined_error): token_metadata_value_type)
  };
}
